...
slavefile=./config/hadoop/slaves
hostfile=./config/system/hosts
authfile=./config/system/authorized_keys
sharefile=/data/share
...
if [ -f $hostfile ]; then
  ...
  echo "192.168.1.10 master" >> $hostfile
fi
...
do
  echo "slave$count"
  echo "slave$count" >> $slavefile
  echo 192.168.1.1$count" "slave$count >> $hostfile
done
...
# 容器和主机可能会与主机共享文件，先建立共享的文件夹（不同的容器的主机名对应不同的文件夹）
echo "creating the namenode master..."
if [ ! -d $sharefile ]; then 
  mkdir -p $sharefile
fi
if [ ! -d $sharefile/master ]; then
  mkdir $sharefile/master
fi

# 后边的配置会用到br0虚拟网桥，如果存在就先将其删除
ip link set br0 down
ip link delete br0
# 删除主机中的~/.ssh/known_hosts文件，不然可能报错： IT IS POSSIBLE THAT SOMEONE IS DOING SOMETHING NASTY!
rm -f  ~/.ssh/known_hosts
# 建立namenode容器，其主机名为master
docker run -it -d -P -p 50070:50070 -p 8088:8088 --name=master --hostname=master --privileged=true --net=none -v /data/share/master:/opt/share  $IMAGE
...
# 建立共享目录
  if [ ! -d $sharefile/$nodename ]; then
    mkdir $sharefile/$nodename
  fi
  docker run -it -d -P --name=$nodename --hostname=$nodename --privileged=true --net=none -v $sharefile/$nodename:$sharefile  $IMAGE
# 设置固定IP  
...
# 先将每个主机中的id_rsa.pub中的内容下载到到此文件夹下，将其中的内容加入到的authorized_keys文件中
# 再将authorized_keys放置到每个容器的 ~/.ssh/中，这样每个容器之间就可以完成免密码登陆
# 脚本download.sh和upload.sh中使用expect命令，来完成登陆过程中不用输入密码，在脚本中事先指定密码。

for ((i=0; i<=$num; i++))
do
  addr=192.168.1.1$i
  ./script/download.sh $addr ~/.ssh/id_rsa.pub ./rsa_tmp
  cat rsa_tmp >> $authfile
  rm -f rsa_tmp
done
...
do
  addr=192.168.1.1$i
  ./script/upload.sh $addr $authfile  ~/.ssh/
  ./script/upload.sh $addr $hostfile  /etc/hosts
done
